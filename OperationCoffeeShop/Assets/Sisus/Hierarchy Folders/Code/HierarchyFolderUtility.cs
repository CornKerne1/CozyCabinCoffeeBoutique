	...
	public static class HierarchyFolderUtility
	{
		...
		internal static void ForceResetTransformStateWithoutAffectingChildren(Transform transform, bool alsoConvertToRectTransform)
		{
			...
			if(alsoConvertToRectTransform)
			{
				...
				rectTransform.hideFlags = GetHierarchyFolderTransformHideFlags(rectTransform);
			}
			else
			{
		...
		public static HideFlags GetHierarchyFolderTransformHideFlags(Transform transform)
			#if DEV_MODE && SHOW_HIERARCHY_FOLDER_TRANSFORM
			=> HideFlags.NotEditable;
			#else
			// Don't hide transform in prefabs or prefab instances to avoid internal Unity exceptions
			=> transform.gameObject.IsPrefabAssetOrInstance() ? HideFlags.HideInInspector | HideFlags.NotEditable : HideFlags.NotEditable;
			#endif
	}
}
#endif